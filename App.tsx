...
// Internal, reliable UUIDv4 generator.
const uuidv4 = () => {
...
async function getKey(pin: string, salt: Uint8Array) {
...
async function encryptVault(items: any, pin: string) {
...
async function decryptVault(storedVault: any, pin: string) {
    try {
...
const useVault = () => {
    ...
    const saveItems = React.useCallback(async (pin: string, newItems: any[]) => {
        try {
    ...
    const addItem = React.useCallback(async (item: any, pin: string) => {
        const newItem = { ...item, id: uuidv4() };
        const currentItems = items; // Capture current items from state
        await saveItems(pin, [...currentItems, newItem]);
    }, [items, saveItems]);

    const updateItem = React.useCallback(async (updatedItem: any, pin: string) => {
        const currentItems = items; // Capture current items from state
        const newItems = currentItems.map((item) => item.id === updatedItem.id ? updatedItem : item);
        await saveItems(pin, newItems);
    }, [items, saveItems]);

    const deleteItem = React.useCallback(async (id: string, pin: string) => {
        const currentItems = items; // Capture current items from state
        const newItems = currentItems.filter((item) => item.id !== id);
        await saveItems(pin, newItems);
    }, [items, saveItems]);
    
    const changePin = React.useCallback(async (oldPin: string, newPin: string) => {
        setIsLoading(true);
        setError(null);
        try {
            const storedVault = loadVault();
            ...
            const currentItems = await decryptVault(storedVault, oldPin);
            
            const newVault = await encryptVault(currentItems, newPin);
            saveVault(newVault);
            
            setItems(currentItems);
        } catch (e: any) {
            console.error("Change PIN failed:", e);
            setError(e.message || "Failed to change PIN. The current PIN may be incorrect.");
            throw e; 
        } finally {
            setIsLoading(false);
        }
    ...
    return { items, isUnlocked, hasPin, error, setError, isLoading, lock, setupPin, unlock, addItem, updateItem, deleteItem, changePin };
};
...
const VirtualKeyboard = ({ onKeyPress }: any) => {
    ...
    const Key = ({ value, label }: any) => (<button onClick={() => handlePress(value)} className="text-2xl font-light text-gray-200 bg-white/5 rounded-lg h-16 flex items-center justify-center transition-colors duration-200 hover:bg-white/10">{label || value}</button>);
...
const LockScreen = ({ isLocked, hasPin, error, isLoading, setupPin, unlock }: any) => {
    ...
    const handlePinChange = (value: string) => { if (value.length <= 6) { setLocalError(null); if(isConfirming) { setConfirmPin(value); } else { setPin(value); } } };
...
const ResetPin = ({ onChangePin, onCancel, isLoading, error }: any) => {
    ...
    const handlePinChange = (value: string) => {
        ...
        setLocalError(null);
        if (stage === 'enter_old') setOldPin(value);
        else if (stage === 'enter_new') setNewPin(value);
        else if (stage === 'confirm_new') setConfirmPin(value);
    };

    const handleContinue = async () => {
        if (stage === 'enter_old') {
            if (oldPin.length < 4) { setLocalError('PIN inválido.' as any); return; }
            setStage('enter_new');
        } else if (stage === 'enter_new') {
            if (newPin.length < 4) { setLocalError('O novo PIN deve ter entre 4 a 6 dígitos.' as any); return; }
            if (newPin === oldPin) { setLocalError('O novo PIN deve ser diferente do antigo.' as any); return; }
            setStage('confirm_new');
        } else if (stage === 'confirm_new') {
            if (newPin !== confirmPin) { setLocalError('Os PINs não correspondem.' as any); setConfirmPin(''); return; }
            setIsProcessing(true);
            try {
                await onChangePin(oldPin, newPin);
                onCancel(); 
            } catch (e) {
                setLocalError('O PIN atual está incorreto.' as any);
    ...
    const handleBack = () => {
        setLocalError(null);
        if (stage === 'confirm_new') { setConfirmPin(''); setStage('enter_new'); } 
        else if (stage === 'enter_new') { setNewPin(''); setStage('enter_old'); } 
        else { onCancel(); }
    };

    const titles: any = { enter_old: 'Redefinir PIN', enter_new: 'Criar Novo PIN', confirm_new: 'Confirmar Novo PIN' };
    const subtitles: any = { enter_old: 'Para sua segurança, insira seu PIN atual.', enter_new: 'Insira um novo PIN de 4 a 6 dígitos.', confirm_new: 'Confirme seu novo PIN.'};
    const currentPin: any = { enter_old: oldPin, enter_new: newPin, confirm_new: confirmPin }[stage];
    const isButtonDisabled = isProcessing || currentPin.length < 4;
    
    return (
        <div>
            <header className="flex items-center justify-between mb-6">
                <button onClick={handleBack} className="p-2 -ml-2 text-gray-400 hover:text-white"><BackIcon /></button>
                <h2 className="text-lg font-bold">{titles[stage]}</h2>
                <div className="w-6 h-6"></div>
            </header>
            <div className="text-center">
                <p className="text-gray-400 mb-6">{subtitles[stage]}</p>
                <div className="flex justify-center space-x-3 my-4">
                    {[...Array(6)].map((_, i) => (<div key={i} className={`w-4 h-4 rounded-full border-2 border-gray-600 ${currentPin.length > i ? "bg-blue-400 border-blue-400" : ""}`}></div>))}
                </div>
                {localError ? <p className="text-red-400 my-4 h-6">{localError}</p> : <div className="my-4 h-6"></div>}
                <VirtualKeyboard key={stage} onKeyPress={handlePinChange} />
                <button onClick={handleContinue} disabled={isButtonDisabled} className="w-full mt-6 bg-blue-600 text-white font-bold py-3 px-4 rounded-lg transition-colors duration-200 enabled:hover:bg-blue-700 disabled:bg-gray-500 disabled:cursor-not-allowed">
                    {isProcessing ? <div className="flex justify-center items-center"><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div></div> : 'Continuar'}
                </button>
            </div>
        </div>
    );
...
const Field = ({ label, value, isSecret = false }: any) => {
...
const ViewItem = ({ item, onBack, onEdit }: any) => {
...
const ItemForm = ({ item, onSave, onCancel, onDelete }: any) => {
    ...
    const handleActionAttempt = (type: string) => { 
...
const Vault = ({ vault }: any) => {
    ...
    const handleSelectItem = (item: any) => { setSelectedItem(item); setCurrentView("VIEW"); };
    const handleEditItem = (item: any) => { setSelectedItem(item); setCurrentView("EDIT"); };
    const handleAddNew = () => { setSelectedItem(null); setCurrentView("ADD"); };
    const handleBack = () => { setSelectedItem(null); setCurrentView("LIST"); vault.error && vault.setError(null) };
    const handleSaveItem = async (item: any, pin: string) => { if ('id' in item) { await vault.updateItem(item, pin); } else { await vault.addItem(item, pin); } handleBack(); }
    const handleDeleteItem = async (id: string, pin: string) => { await vault.deleteItem(id, pin); handleBack(); }
    const handleInitiateResetPin = () => { setCurrentView("RESET_PIN"); };
    
    const renderView = () => { switch (currentView) { case "VIEW": return selectedItem && <ViewItem item={selectedItem} onBack={handleBack} onEdit={() => handleEditItem(selectedItem)} />; case "ADD": case "EDIT": return <ItemForm item={selectedItem} onSave={handleSaveItem} onCancel={handleBack} onDelete={handleDeleteItem} />; case "RESET_PIN": return <ResetPin onChangePin={vault.changePin} onCancel={handleBack} isLoading={vault.isLoading} error={vault.error} />; case "LIST": default: return <ItemList items={vault.items} onSelectItem={handleSelectItem} onAddNew={handleAddNew} />; } };
    
    return (<div className="bg-[#161b22] border border-gray-800 rounded-lg p-6 shadow-lg w-full max-w-md min-h-[600px] flex flex-col">
        {currentView === 'LIST' && (
            ...
            <div className="flex items-center space-x-2">
                <button onClick={handleInitiateResetPin} className="text-gray-400 hover:text-white" aria-label="Redefinir PIN"><SettingsIcon /></button>
                <button onClick={vault.lock} className="text-gray-400 hover:text-white" aria-label="Trancar cofre"><LockIcon /></button>
            </div>
            </header>
        )}
